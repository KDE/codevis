...
def is_valid_build_path(path):
    ...
    return os.path.isfile(f'{dirname}/lakospreferences_export.h')


def is_valid_src_path(path):
    import os
    dirname = os.path.dirname(path)
    return os.path.isfile(f'{dirname}/lakosdiagram.desktop')
...
def accept(path):
    ...
    if '/lakosdiagramadaptor' in path:
        return True
...
def process(path, addPkg):
    ...
    elif '/lakosdiagramadaptors' in path:
        pkgName = 'lvt-desktopapp'
    else:
