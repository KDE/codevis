...
public:
    ...
    QColor colorForType(KMessageWidget::MessageType type) const;
};

QColor KMessageWidgetPrivate::colorForType(KMessageWidget::MessageType) const
{
    QColor bgBaseColor;
...
void KMessageWidget::setText(const QString &text)
{
    ...
    d->bgColor = d->colorForType(messageType());
    updateGeometry();
}
...
void KMessageWidget::animatedShow()
{
    if (isVisible() && height() != 0) {
        ...
        anim->setEndValue(d->colorForType(d->messageType));
...
QColor KMessageWidget::bgColor() const
{
    return d->bgColor;
}

void KMessageWidget::setBgColor(const QColor& color)
{
    if (d->bgColor == color) {
        return;
    }
